                                                ...
                                                <div className={`transition-all duration-300 ease-in-out ${activeTab === 'payment' ? 'opacity-100 visible translate-y-0 relative' : 'opacity-0 invisible -translate-y-2 absolute inset-0 pointer-events-none'}`}>
                                                    {orderLoading && !orderData ? (
                                                        ...
                                                        filtered.forEach(item => {
                                                            const bId = item.isBatch ? item.batchId : item._id
                                                            const t = new Date(item.createdAt).getTime()
                                                            if (!batchTimes[bId] || t > batchTimes[bId]) {
                                                                batchTimes[bId] = t
                                                            }
                                                        })
                                                        ...
                                                        const sortedFiltered = [...filtered].sort((a, b) => {
                                                            ...
                                                            if (a.isPaid && b.isPaid) {
                                                                ...
                                                                if (a.paymentLogId !== b.paymentLogId) return (a.paymentLogId || "").localeCompare(b.paymentLogId || "")
                                                            }
                                                            
                                                            const aBatch = a.isBatch ? a.batchId : a._id
                                                            const bBatch = b.isBatch ? b.batchId : b._id
                                                            if (aBatch !== bBatch) {
                                                                if (batchTimes[aBatch] !== batchTimes[bBatch]) {
                                                                    return batchTimes[bBatch] - batchTimes[aBatch]
                                                                }
                                                        ...
                                                        const totalAssigned = sortedFiltered.reduce((s, e) => s + (e.quantity || 0), 0)
                                                        const totalCleaned = sortedFiltered.reduce((s, e) => s + (e.cleanedQuantity || 0), 0)

                                                        const unpaidBatches = sortedFiltered.filter(a => a.isReturned && !a.isPaid)
                                                                                    ...
                                                                                    <div className="max-h-[360px] overflow-y-auto">
                                                                                        <table className="w-full text-left border-collapse table-fixed">
                                                                                            <thead className="bg-gray-50 text-gray-600 capitalize text-xs font-semibold tracking-wider sticky top-0 z-10">
                                                                                                <tr>
                                                                                                    <th className="px-2 lg:px-3 py-3 w-[24%]">Date & Time</th>
                                                                                                    <th className="px-2 lg:px-3 py-3 w-[15%]">Assigned</th>
                                                                                                    <th className="px-2 lg:px-3 py-3 w-[15%]">Cleaned</th>
                                                                                                    <th className="px-2 lg:px-3 py-3 w-[22%]">Status</th>
                                                                                                    <th className="px-2 lg:px-3 py-3 w-[24%] text-right pr-4">Action</th>
                                                                                                </tr>
                                                                                            </thead>
                                                                                            <tbody className="divide-y divide-gray-100 text-gray-700 text-sm">
                                                                                                {renderItems.map((item) => {
                                                                                                    if (item.isSeparator) {
                                                                                                        return (
                                                                                                            <tr key={item.key} className="bg-green-50/70 border-y-2 border-green-200">
                                                                                                                <td className="px-2 lg:px-3 py-2.5 font-bold text-green-700 align-middle">
                                                                                                                    <div className="whitespace-nowrap">Total Paid</div>
                                                                                                                    <div className="text-[10px] text-green-600 font-semibold whitespace-nowrap">{new Date(item.date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</div>
                                                                                                                </td>
                                                                                                                <td className="px-2 lg:px-3 py-2.5 font-bold text-green-700 align-middle whitespace-nowrap">{item.assigned} KG</td>
                                                                                                                <td className="px-2 lg:px-3 py-2.5 font-bold text-green-700 align-middle whitespace-nowrap">{item.cleaned} KG</td>
                                                                                                                <td className="px-2 lg:px-3 py-2.5 align-middle">
                                                                                                                    <span className="inline-block px-2 py-0.5 rounded text-[11px] font-bold bg-gray-100 text-gray-700 border border-gray-200 whitespace-nowrap">Payment Done</span>
                                                                                                                </td>
                                                                                                                <td className="px-2 lg:px-3 py-2.5 text-right pr-4 align-middle">
                                                                                                                    <span className="text-green-700 font-black text-base md:text-lg whitespace-nowrap">Paid: ₹{item.totalAmount}</span>
                                                                                                                </td>
                                                                                                            </tr>
                                                                                                        )
                                                                                                    }
                                                                                                    ...
                                                                                                    return (
                                                                                                        ...
                                                                                                        >
                                                                                                            <td className="px-2 lg:px-3 py-3 font-medium align-middle">
                                                                                                                <div className="whitespace-nowrap">{new Date(entry.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</div>
                                                                                                                <div className="text-[11px] text-gray-400 mt-0.5 whitespace-nowrap">{new Date(entry.createdAt).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: true })}</div>
                                                                                                            </td>
                                                                                                            <td className="px-2 lg:px-3 py-3 text-gray-700 align-middle whitespace-nowrap">
                                                                                                                {entry.quantity ? `${entry.quantity} KG` : "—"}
                                                                                                            </td>
                                                                                                            {isFirstInBatch && (
                                                                                                                <>
                                                                                                                    <td className="px-2 lg:px-3 py-3 font-semibold text-gray-700 relative align-middle" rowSpan={batchGroup.length}>
                                                                                                                        {batchGroup.length > 1 && (
                                                                                                                        ...
                                                                                                                        <div className={batchGroup.length > 1 ? "pl-4 lg:pl-5 whitespace-nowrap" : "whitespace-nowrap"}>
                                                                                                                            {displayCleanedQuantity ? `${displayCleanedQuantity} KG` : "—"}
                                                                                                                        </div>
                                                                                                                    </td>
                                                                                                                    <td className="px-2 lg:px-3 py-3 align-middle" rowSpan={batchGroup.length}>
                                                                                                                        {isPaid ? (
                                                                                                                            <span className="inline-block px-2 py-0.5 rounded text-[11px] font-bold bg-gray-100 text-gray-700 border border-gray-200 whitespace-nowrap">Payment Done</span>
                                                                                                                        ) : isSelected ? (
                                                                                                                            batchGroup.some(g => g._id === selectedRowId) ? (
                                                                                                                                <span className="inline-block px-2 py-0.5 rounded text-[11px] font-bold bg-orange-100 text-orange-700 border border-orange-200 animate-pulse whitespace-nowrap">Paying...</span>
                                                                                                                            ) : (
                                                                                                                                <span className="inline-block px-2 py-0.5 rounded text-[11px] font-semibold bg-orange-50 text-orange-600 border border-orange-100 whitespace-nowrap">Selected</span>
                                                                                                                            )
                                                                                                                        ) : entry.isReturned ? (
                                                                                                                            <span className="inline-block px-2 py-0.5 rounded text-[11px] font-semibold bg-red-50 text-red-700 border border-red-100 whitespace-nowrap">Unpaid</span>
                                                                                                                        ) : (
                                                                                                                            <span className="inline-block px-2 py-0.5 rounded text-[11px] font-medium bg-gray-50 text-gray-400 border border-gray-100 whitespace-nowrap">Pending Return</span>
                                                                                                                        )}
                                                                                                                    </td>
                                                                                                                    <td className="px-2 lg:px-3 py-3 text-right pr-4 align-middle" rowSpan={batchGroup.length}>
                                                                                                                        {!isPaid && entry.isReturned && (
                                                                                                                            <div className="flex justify-end">
                                                                                                                                {selectedRowId ? (
                                                                                                                                    batchGroup.some(g => g._id === selectedRowId) ? (
                                                                                                                                        <button className="px-3 py-1 bg-orange-500 text-white rounded-lg text-xs font-bold shadow-sm whitespace-nowrap">Paying</button>
                                                                                                                                    ) : isSelected ? (
                                                                                                                                        <span className="text-xs text-orange-500 font-semibold whitespace-nowrap">—</span>
                                                                                                                                    ) : (
                                                                                                                                        <button onClick={(e) => { e.stopPropagation(); handleRowClick(entry, local.LocalID); }} className="px-3 py-1 bg-white border border-gray-300 text-gray-700 rounded-lg text-xs font-bold hover:bg-gray-50 transition-colors whitespace-nowrap">Pay</button>
                                                                                                                                    )
                                                                                                                                ) : (
                                                                                                                                    <button onClick={(e) => { e.stopPropagation(); handleRowClick(entry, local.LocalID); }} className="px-3 py-1 bg-orange-500 text-white rounded-lg text-xs font-bold shadow-sm hover:bg-orange-600 transition-colors whitespace-nowrap">Pay</button>
                                                                                                                                )}
                                                                                                                            </div>
                                                                                                                        )}
                                                                                                                    </td>
